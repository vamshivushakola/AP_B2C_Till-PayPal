...
public class PayPalHopPaymentRequestController extends AbstractCheckoutStepController
{
	 ...
	 * (non-Javadoc)
	 *
	 * @see de.hybris.platform.storefront.controllers.pages.checkout.steps.CheckoutStepController#enterStep(org.
	 * springframework .ui.Model, de.hybris.platform.storefront.controllers.pages.checkout.steps.RedirectAttributes)
	 */
	@Override
	...
	private boolean easyPaymentsSelected()
	{
		return Boolean.TRUE.toString()
				.equalsIgnoreCase(configurationService.getConfiguration().getString(PaypalConstants.USE_EASY_PAYMENT))
				&& Boolean.TRUE.equals(getSessionService().getAttribute(PaypalConstants.IS_PAYPAL_CREDIT))
				&& !PaypalConstants.AUTHORIZATION_PAYMENT_ACTION_NAME
						.equalsIgnoreCase(configurationService.getConfiguration().getString(PaypalConstants.PAYMENT_ACTION));
	}
	...
	private boolean redirectToLoginPage(final Model model)
	{
		...
		final boolean isGuestUserRedirect = configurationService.getConfiguration()
				.getBoolean(PaypalConstants.PAYPAL_GUEST_REDIRECT);
			...
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		final Map<String, String> errorsDetails = getSessionService()
				.getAttribute(PaypaladdonControllerConstants.PAY_PAL_ERRORS_DETAILS);
